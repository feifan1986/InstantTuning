...
class GeneticAlgorithm(object):
    """GA class"""
    def __init__(self, chromNum, eliteRate, muteRate, stageNum, rmsThres, 
                 signalpath, selectmethod='rms_ranking', save=True):
    ...
    def run(self, parmMin, parmMax, parmSamples, model):
        ...
        while not self.check_stop(stageIdx):
            if DEBUG:
                print "stage %d, jobparms = \n%s" % (stageIdx, str(jobparms))
...
class Population(object):
    ...
    def elite_selection(self, eliteRate=0):
        ...
        eliteIndexes = np.argsort(unique_rmses)[:eliteNum] # model of top N
        eliteIndexes = [unique_indices[ii] for ii in eliteIndexes]

        self.matepoolNum -= eliteNum
        self.rmses = rmses
        self.elitepool = eliteIndexes[:]
        self.singlepool = [ii for ii in unique_indices if ii not in eliteIndexes]
        if DEBUG:
            print 'rms = %s\nelite rms=%s\n singlepool rms = %s' % (str(rmses), str([rmses[ii] for ii in eliteIndexes]), str([rmses[ii] for ii in self.singlepool]))
    ...
    def perform_ga_operations(self, eliteRate, muteRate, parmMin, parmMax, parmSamples, parmNames, parmBound, stageIdx, selectmethod='rms_ranking'):
        '''
        1. elite selection
...
if __name__ == "__main__":
    cwd = os.getcwd()
    datapath = os.path.join(cwd, "data", "HOD0")
    ...
    #################### Modify resist template here
    useterms = ['AG1', 'Bp', 'Am', 'Bn', 'Slope', 'DE1', 'MGS1', 'MGS']
    ...
    # for method in ['linear_ranking', 'tournament', 'rms_ranking']: # 'rms_ranking',
        ################ Modify GA settings here
        ga = GeneticAlgorithm(chromNum=50, eliteRate=0.02, muteRate=0.08, stageNum=10, 
                              rmsThres = 0.3, signalpath=datapath, selectmethod=method)
        ###########################################
