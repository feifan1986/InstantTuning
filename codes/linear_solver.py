...
class Solver(object):
    ...
    def specErrors(self):
        ...
        if anchor.empty:
            anchor = validgauge.query('{}=="Anchor"'.format(self.GROUPNAME))
        if anchor.empty:
            anchor = np.nan
...
if __name__ == '__main__':
    ...
    #'''
    datapath = os.path.join(curpath, "data", 'HOD0')
    ...
    rms, outmodel, calresult, groupstat = s.fit(model, method='HOD0')
